...
#define MAX_REPLY_BUFFER_SIZE \
    (REPLY_BUFFER_SIZE - 1)  // at least 1 termination char should be always present in the buffer
...
{
    ROBOTTO_ASSERT_DEBUG(timer != NULL);
...
static void makeNewTxRequest(const CommunicationEventData *data)
{
    ...
    // crop message if it's too long to fit buffer and termination chars. Should
    // never happen
