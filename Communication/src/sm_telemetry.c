...
// TODO: remove hardcoded reference to queue handles
...
{
    telemetry_timer = xTimerCreate("telemetry_timer",
...
TelemetryState onTelemetryTick(const CommunicationEventData *data)
{
    ...
    if (pdTRUE == xQueuePeek(robotto_motion_telemetry_queue_handle,
                             &aggregated_telemetry.motion_telemetry,
                             0))
    {
        xQueuePeek(robotto_object_detection_telemetry_queue_handle,
...
static const TelemetryStateTransitionFunctionPtr
    telemetry_state_transition_table[TELEMETRY_STATE_COUNT][EVENT_COUNT] = {
        [TELEMETRY_STATE_OFF] =
            {
...
{
    TelemetryStateTransitionFunctionPtr function =
        telemetry_state_transition_table[state][event->id];
    if (function != NULL)
    {
