...
static UartTxState onUartTxRequest(const CommunicationEventData *data)
{
    ...
    HAL_StatusTypeDef result =
        HAL_UART_Transmit_DMA(&H_UART_ESP, data->uart_tx.buffer, data->uart_tx.size);
...
static const UartTxTransitionFunction
    uart_tx_state_transition_table[UART_TX_STATUS_COUNT][EVENT_COUNT] = {
        [UART_TX_STATUS_IDLE] =
            {
