                                    ...
                                    int out_buffer_size)
{
    ...
    written =
        serializeDetectionTelemetryBinary(detection_telemetry, out + off, out_buffer_size - off);
    if (written < 0)
        return -1;
    off += written;
                                  ...
                                  int max_out_size)
{
    ...
    int payload_len = serializeTelemetryBinary(
        pose, target_pose, speed_set_point, detection_telemetry, payload, sizeof(payload));
    if (payload_len < 0)
    {
        return -1;
    }
    ...
    off += snprintf(out, max_out_size, "AT+MQTTPUB=0,\"%s\",\"", topic);

    off += snprintf(out + off, max_out_size - off, "%s", payload_b64);

    off += snprintf(out + off, max_out_size - off, "\",0,0");
