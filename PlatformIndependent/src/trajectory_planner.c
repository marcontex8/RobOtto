...
{
    float euclidean_distance = sqrtf((current->x - end_pose.x) * (current->x - end_pose.x) +
                                     (current->y - end_pose.y) * (current->y - end_pose.y));
...
WheelSpeedSetPoint computeWheelSpeedSetpoint(const RobottoPose *current)
{
    ...
    float distance = sqrtf(target_vector_x * target_vector_x + target_vector_y * target_vector_y);
    ...
    else
    {
        ...
        float alpha =
            wrapToPI(atan2f(current->x - lookahead_x, lookahead_y - current->y) - current->theta);
