...
static RobottoPose targetPoses[4] = {
    {.x = 0.0, .y = 1.0}, {.x = 1.0, .y = 1.0}, {.x = 1.0, .y = 0.0}, {.x = 0.0, .y = 0.0}};
...
ActivityStatus motionPlanningStatusRunning()
{
    ...
    RobottoPose estimated_pose;
    if (pdTRUE != xQueuePeek(robotto_pose_queue_handle, &estimated_pose, 0) ||
        (xTaskGetTickCount() - estimated_pose.timestamp) > MAX_ESTIMATED_POSE_DELAY)
    {
    ...
    RobottoMotionTelemetry motion_telemetry = {.pose = estimated_pose,
