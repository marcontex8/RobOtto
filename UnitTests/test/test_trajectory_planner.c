 ...
 *
 * The tests are designed to verify the correctness of the trajectory planner's
 * behavior in various scenarios. The control is assumed to be periodic, and the
 * tests will simulate a sequence of pose updates to evaluate the planner's
 * response. A basic mathematical model of the robot's kinematics will be used
 * to calculate the pose according to the wheel speed setpoints. The tests will
 ...
 * Test will pass if the simulated model is able to reach the target pose within
...
void test_EndPoseReachedDetection()
{
    ...
    RobottoPose near_target = {.x = 1.0f + TOLERANCE / 2,
                               .y = 2.0f + TOLERANCE / 2,
                               .theta = M_PI / 6.0f + TOLERANCE / 2,
                               .timestamp = 0};
